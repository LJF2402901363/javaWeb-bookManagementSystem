...
public class BookListServlet extends HttpServlet {
    BookService bookServiceService = new BookServiceImpl();
    ...
    //获取每页的数量
        ...
        if (tableSearch!=null&&tableSearch.trim().length()!=0){
            ...
            //搜索查询，对每一列都进行模糊查询
            bookPageBean= this.bookServiceService.fuzzySearchABookByPage(offset,pageSize,tableSearch,tableSortName,tableOrder);
        }else{
            //根据条件进行模糊查询并显示分页数据
            ...
            //条件查询
            bookPageBean= this.bookServiceService.findABookByPage(offset,pageSize,condition,tableSortName,tableOrder);
        }
